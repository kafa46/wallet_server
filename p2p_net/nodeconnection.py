...
class NodeConnection(threading.Thread):
    """The class NodeConnection is used by the class Node and represent the TCP/IP socket connection with another node.
       ...
       connection.
        ...
        ip: The ip of the main node.
        port: The port of the server of the main node."""

    def __init__(self, main_node, sock, id, ip, port):
        ...
        self.main_node.debug_print("NodeConnection: Started with client (" + self.id[:10] + ") '" + self.ip + ":" + str(self.port) + "'")
    ...
    def __str__(self):
        return 'NodeConnection: {}:{} <-> {}:{} ({})'.format(self.main_node.ip, self.main_node.port, self.ip, self.port, self.id)

    def __repr__(self):
        return '<NodeConnection: Node {}:{} <-> Connection {}:{}>'.format(self.main_node.ip, self.main_node.port, self.ip, self.port)
